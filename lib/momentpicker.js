...
View = (function(superClass) {
  ...
  View.prototype._bind = function() {
    this._bindEl();
    this._bindPanel();
    return $(document).on("mousedown.momentpicker_" + this.id, (function(_this) {
      return function(e) {
...
MomentPicker = (function(superClass) {
  ...
  MomentPicker.prototype.setMoment = function(moment) {
    return this.moment = moment(moment, this.opts.valueFormat);
  };
...
Timeview = (function(superClass) {
  ...
  Timeview.prototype._setActive = function() {
    Timeview.__super__._setActive.call(this);
    this.panel.find("a.panel-item[data-type='hour'][data-value='" + (this.moment.hour()) + "']").addClass('selected');
    this.panel.find("a.panel-item[data-type='hour'][data-value='" + (moment().hour()) + "']").addClass('cur');
    return this.panel.find("a.panel-item[data-type='minute'][data-value='" + (this.moment.format('mm')) + "']").addClass('selected');
  };

  Timeview.prototype._menuItemHandler = function(e) {
    var action, num;
    action = $(e.currentTarget).data('action');
    num = action === 'next' ? 1 : -1;
    this.moment.add(num, 'year');
  ...
  Timeview.prototype._panelItemHandler = function(e) {
    var $currentTarget, type, value;
    $currentTarget = $(e.currentTarget);
    type = $currentTarget.data('type');
    value = $currentTarget.data('value');
    if (type === 'hour') {
      this.moment.set('hour', value);
      $('.selected').removeClass('selected');
      $currentTarget.addClass('selected');
      return this._setElValue();
    } else if (type === 'minute') {
      this.moment.set('minute', value);
      this._setElValue();
      return this.hide();
    }
  };

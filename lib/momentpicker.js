...
View = (function(superClass) {
  ...
  View.prototype.select = function(value, refreshInput, finished) {
    ...
    if (this.name === 'date') {
      ...
      if (moment.format('M') !== this.moment.format('M')) {
        this.moment.set('month', moment.format('M') - 1);
      }
...
DateView = (function(superClass) {
  ...
  DateView.prototype.select = function(value, refreshInput, finished) {
    var tmpDate;
    tmpDate = moment(this.currentMonth, 'YYYY-MM');
    this.moment.set('date', value);
    this.moment.set('month', tmpDate.get('month'));
    this.moment.set('year', tmpDate.get('year'));
    if (this.timer) {
      clearTimeout(this.timer);
    }
    this._refreshSelected();
